...
void initPriorityQueue(std::vector<CodeTableElement>& vCodeTable, std::priority_queue<QueueElement>& voPriorityQueue, std::vector<HuffmanTreeNode>& voHuffmanTree);
...
void executeHuffmanEncoding(const std::vector<int>& vInput, std::vector<bool>& voOutput)
{
	...
	generateFrequencyTable(vInput, CodeTable);
	...
	//generateHuffmanCode(CodeTable);
	//generateEncodedBitStream(vInput, CodeTable, voOutput);
}
...
int main()
{
	...
	std::vector<std::pair<float, float>> ClusterCenter, OutputData;
